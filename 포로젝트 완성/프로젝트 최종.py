...
class GUIMODE():
    ...
    def packet_temp(self):  # 서버에 네트워크 인터페이스를 host only로 설정하여 호스트pc만 통신할수있도록 설정
                            # 그리고 그것을 캡처할수있도록 sniff 사용  count 1000  번 캡처할수있도록
        packet_result = sniff(iface="VMware Network Adapter VMnet1", filter='ip', prn=self.showPacket, count=1000)
        wrpcap("packet.pcap", packet_result)  # packet.pcap파일을 exe 프로그램이 있는 폴더에 저장

    def file_packet(self):
        f = open("packet.txt", "w")   # 패킷정보들을 exe 프로그램이 있는 폴더에 packet.txt파일로 저장
        sys.stdout = f
